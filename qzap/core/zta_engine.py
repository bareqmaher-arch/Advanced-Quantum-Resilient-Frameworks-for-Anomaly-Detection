...
class PolicyEngine:
    ...
    # Supported condition patterns (avoids eval() for security)
    _CONDITION_MAP = {
        "risk_level == 'critical'": lambda ra, _: ra.risk_level == RiskLevel.CRITICAL,
        "risk_level == 'high'":     lambda ra, _: ra.risk_level == RiskLevel.HIGH,
        "risk_level == 'medium'":   lambda ra, _: ra.risk_level == RiskLevel.MEDIUM,
        "risk_level == 'low'":      lambda ra, _: ra.risk_level == RiskLevel.LOW,
        "risk_score >= 0.8":        lambda ra, _: ra.risk_score >= 0.8,
        "risk_score >= 0.6":        lambda ra, _: ra.risk_score >= 0.6,
        "risk_score >= 0.3":        lambda ra, _: ra.risk_score >= 0.3,
        "anomaly_score >= 0.7":     lambda ra, _: ra.anomaly_score >= 0.7,
    }
    ...
    def _evaluate_condition(
        ...
        """Evaluate a policy condition safely using a pre-defined lookup map."""
        handler = self._CONDITION_MAP.get(condition.strip())
        if handler is not None:
            return handler(risk_assessment, request)
        logger.error(f"Unknown policy condition: '{condition}'. Defaulting to False.")
        return False
